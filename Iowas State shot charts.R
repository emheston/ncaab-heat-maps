...
# NCAA data
# Iowa State schedule
isu_schedule <- get_schedule("Iowa State", season = "2021-22")
# Iowa State game ids
isu_ids <- isu_schedule$game_id[1:35]
# Iowa State all shots
isu_shots <- get_shot_locs(isu_ids)
# get Tyrese Hunter's shots
tyrese <- isu_shots %>% filter(shooter=="Tyrese Hunter", game_id > 401400000)
#Change shot data to fit court dimensions
tyrese <- tyrese %>% mutate(x=x-25)

# NCAA chart
p1 <- plot_court(court_themes$ppt, use_short_three = T) +
  geom_point(data = tyrese, aes(x = x, y = y, color = tyrese$outcome, fill = tyrese$outcome), 
  ...
  ggtitle(label = "Tyrese Hunter Postseason",
          subtitle = "Big 12 Tournament Game and Sweet Sixteen Run")
