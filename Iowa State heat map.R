...
# NCAA data
# Iowa State schedule
sbu_schedule <- get_schedule("St Bonaventure", season = "2021-22")
# Iowa State game ids
sbu_ids <- sbu_schedule$game_id[1:33]
# Iowa State all shots
sbu_shots <- get_shot_locs(sbu_ids)
# get Tyrese Hunter's shots
holmes <- sbu_shots %>% filter(shooter=="Jaren Holmes")
holmes <- holmes %>% mutate(x=x-25)
# NCAA heatmap
palette <- paletteer::paletteer_d("RColorBrewer::PuBu", direction = 1)
p1 <- plot_court(court_themes$ppt) + 
  geom_density_2d_filled(holmes, mapping = aes(x=x,y=y,fill = ..level..,), 
                         contour_var = "ndensity", breaks = seq(0.03, 1.0, length.out = 10), alpha = .5)  + 
  ...
  labs(title = "Holmes Shot Heatmap") 
